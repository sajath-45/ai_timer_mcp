...
//uses one agent
export async function runWorkflowFastV2(
  workflow: WorkflowInput
): Promise<WorkflowResponse> {
  if (!workflow.input_as_text) {
    throw new Error("input_as_text is required");
  ...
  if (!keyword) {
    const output = {
      ...
      confidence: "low",
      reasoning:
        "Could not confidently extract a property keyword from transcript",
      alternatives: [],
    };
  ...
  const search = await searchPropertyFuzzyNew(keyword, 5);
  const top = search.matches?.[0];
  const topScore: number = Number(top?.combined_score ?? top?.db_score ?? 0);

  const confidence =
    topScore >= 75 ? "high" : topScore >= 50 ? "medium" : "low";
  ...
  console.log(
    `[${new Date().toISOString()}] Fast workflow completed in ${
      Date.now() - startedAt
    }ms (keyword='${keyword}', topScore=${topScore})`
  );
...
export async function runWorkflowHybrid(
  input: WorkflowInput
): Promise<WorkflowResponse> {
  ...
  if (topScore >= 70) {
    ...
    console.log(
      `[${new Date().toISOString()}] Hybrid (fast path): ${
        Date.now() - start
      }ms`
    );
    return { output_text: JSON.stringify(output, null, 2) };
  ...
  console.log(
    `[${new Date().toISOString()}] Low confidence (${topScore}), using LLM...`
  );
  return runWorkSmart(input);
}

export async function runWorkSmart(
  input: WorkflowInput
): Promise<WorkflowResponse> {
  const start = Date.now();
  ...
  console.log(
    `[${new Date().toISOString()}] Accurate workflow: ${Date.now() - start}ms`
  );
