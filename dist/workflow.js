...
exports.runWorkflowOptimized = exports.runWorkflow = void 0;
...
const runWorkflow = async (workflow) => {
    ...
    console.log(`[${new Date().toISOString()}] runWorkflow start with input_as_text:`, workflow.input_as_text);
    return await (0, agents_1.withTrace)("Get Object ID", async () => {
        ...
        const extractpropertynameResultTemp = await runner.run(extractpropertyname, [...conversationHistory]);
        conversationHistory.push(...extractpropertynameResultTemp.newItems.map((item) => item.rawItem));
        ...
        console.log(`[${new Date().toISOString()}] ExtractPropertyName output:`, extractpropertynameResult.output_text);
        ...
        conversationHistory.push(...propertyAgentResultTemp.newItems.map((item) => item.rawItem));
        ...
        console.log(`[${new Date().toISOString()}] PropertyAgent output (duration enforced):`, finalOutput);
